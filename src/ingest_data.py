...
import pandas as pd
"""
1. abstract class makes interface for multiple required strategies
2. then make different strategy classes
3. then make context class for the strategies like when to use which strategy
...
class ZipDataIngestor(DataIngestor):
    def ingest(self, file_path: str) -> pd.DataFrame:
        ...
        csv_files.sort(key=lambda f: os.path.getmtime(os.path.join("extracted_data", f)), reverse=True)
...
if __name__ == "__main__":
    ...
    """
    Example usage is for testing and understanding purpose only.
    steps files of zenml are used to make the pipeline flow.
    """
